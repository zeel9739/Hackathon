...
export default function Chennal() {
  ...
  const getChannelApi = () => {
    axios
      .get(`http://localhost:3004/channel/get/channel`)
      ?.then((respChannel) => {
        console.log("respChannel", respChannel);
        setGetChannel(respChannel.data.channels);
      });
  };
  useEffect(() => getChannelApi(), []);
  ...
  const handalDeleteModalYesBtn = () => {
    ...
    axios
      .delete(`http://localhost:3004/channel/delete/channel/${deleteRecipe}`)
      .then((delData) => {
        console.log("DeleteChannel===>", delData);
        getChannelApi();
      });
  ...
  // const onChangeHandeler = (getChannel) => {
  //   let matches = [];
  //   console.log("getChannel==========>", getChannel);
  //   if (getChannel.length > 0) {
  //     matches = getChannel?.filter((user) => {
  //       console.log(user, "user=======----------");
  //       const regex = new RegExp(`${getChannel}`, "m");
  //       return user?.getChannel.match(regex);
  //     });
  //   }
  ...
  const [foundUsers, setFoundUsers] = useState(getChannel);

  useEffect(() => {
    setFoundUsers(getChannel);
  }, [getChannel]);

  const onChangeHandeler = (e) => {
    console.log("getChannel onChangeHandeler", getChannel);
    const keyword = e.target.value;
    console.log("bhavy", keyword);

    if (keyword !== "") {
      const results = getChannel.filter((user) => {
        console.log(user, "user");
